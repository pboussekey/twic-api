    ...
    public function getListId($me_id, $page_id = null, $user_id = null, $parent_id = null, $is_item = null, $is_admin = false)
    {
        ...
        if (null !== $parent_id) {
            ...
            $select->join('subscription', 'subscription.libelle=post_subscription.libelle', [], $select::JOIN_LEFT)
                ...
                ->where([' ((page.type <> "course" OR page.is_published IS TRUE OR page_user.role = "admin") AND page_user.user_id IS NOT NULL AND (page_user.state NOT IN ("pending", "invited") OR (page_user.state = "invited" AND post.type = "page")) ))'], Predicate::OP_OR);
        }
